...
fn apply(cmd: &Command, perm: &mut Permutation) {
    ...
    match instr {
        CMP => {
            ...
            perm.flags[0] = perm.perm[to] < perm.perm[from];
            perm.flags[1] = perm.perm[to] > perm.perm[from];
        }
        // MOV => perm[to] = perm[from],
        MOV => perm.perm[to] = perm.perm[from],
        CMOVG => {
            // if perm[REGS + 1] == 1 {
            //     perm[to] = perm[from];
            // }
            if perm.flags[1] {
                perm.perm[to] = perm.perm[from];
            }
        }
        CMOVL => {
            // if perm[REGS + 0] == 1 {
...
fn perm_positions(perm: &Permutation) -> PermInfo {
    let mut pos : [Vec<u8>; NUMBERS] = Default::default();
    for (i, &n) in perm.perm.iter().enumerate() {
        if n > 0 {
    ...
    pos.sort();
    // let flags = perm.flags.iter().map(|&x| x == 1).collect();
    PermInfo{perm: pos, flags: perm.flags}
}
...
fn main() {
    ...
    let initial_state: State = permutations
        ...
        .map(|p| {
            ...
            // as struct
            let mut perm = Permutation{perm: [0; REGS], flags: [false; 2]};
            for (i, &x) in p.iter().enumerate() {
                perm.perm[i] = x;
            }
            perm
    ...
    while length<20 {
        ...
        let solutions = 
            ...
            .filter(|(_,state)| state.iter().all(|p| p.perm[0..NUMBERS] == state[0].perm[0..NUMBERS]))
            .collect::<Vec<_>>();
        if solutions.len() > 0 {
