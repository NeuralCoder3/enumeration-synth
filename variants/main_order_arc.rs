...
fn apply_functional(cmd: &Command, perm: &Permutation) -> Permutation {
    // same as above but functional (without cloning)
    let (instr, to, from) = *cmd;
    match instr {
        CMP => {
            perm.iter().enumerate().map(|(i, &x)| {
                if i == REGS + 0 {
                    (perm[to] < perm[from]) as u8
                } else if i == REGS + 1 {
                    (perm[to] > perm[from]) as u8
                } else {
                    x
        ...
        MOV => perm.iter().enumerate().map(|(i, &x)| {
            if i == to {
                perm[from]
            } else {
                x
...
fn apply_all(cmd: &Command, state: &State) -> State {
    ...
    for perm in state {
        // no difference functional vs. imperative
        let new_perm = apply_functional(cmd, perm);
